...
class Zencoder extends Services_Zencoder
{
    ...
    {
        return $this->jobs()->details($jobID);
    }

    public function transcode($input, $transcoder, $meta = [])
    {
        $settingsClass = config('zencoder.transcode.'.$transcoder);

        $settings = new $settingsClass;

        return $this->jobs()->create($settings->settings($input, $meta));
    }
}
